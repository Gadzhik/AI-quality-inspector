...
DEFECT_FRAMES = 125     # длительность эпизода (~5с при 25fps) — длина сигнала тревоги
                        # в манифест-режиме. Не трогаем: 5с = требуемая длительность.
GAP_FRAMES_MIN = 20     # КОРОТКИЕ паузы: высокое покрытие (~80% кадров дефектные) при
GAP_FRAMES_MAX = 45     # сохранении 5с-эпизодов. Нужно для VLM-режима: медленный
                        # инференс (~50с/кадр) семплит редко, поэтому почти каждый
                        # кадр должен быть дефектным, чтобы выборка попадала на дефект.
                        # Манифест-режим от плотности не страдает (мгновенный lookup).
