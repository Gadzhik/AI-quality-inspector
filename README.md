...
```
.
├── main.py                       # Демо-режим: детекция по манифесту (мгновенно)
├── main_vlm.py                   # VLM-режим: живой инференс LM Studio (медленно, реально)
...
**Демо-режим** (по умолчанию) — детекция по манифесту, мгновенно, без LM Studio:

```bash
python main.py
```

**VLM-режим** (`main_vlm.py`) — живой инференс через LM Studio вместо манифеста.
Каждый кадр реально анализирует модель: детекция, тип дефекта, бокс и уверенность
приходят из VLM, а не из ground-truth.

**Предусловия:**
1. Запущен **LM Studio Local Server** (порт `1234`) с загруженной vision-моделью
   `google/gemma-4-12b-qat` и активным **Vision Adapter**.
2. Сгенерировано демо-видео (`python create_realistic_defects.py`).

```bash
python main_vlm.py
```

Поведение: анализ крутится в фоне (новый старт — только после завершения
предыдущего), при дефекте кадр **замораживается** и на него ложится рамка — иначе
за время инференса живое видео ушло бы вперёд. Клавиши `s`/`d`/`q` те же.

#### Manifest-guided семплинг

VLM медленная (~30 с/кадр), и если подавать ей **случайный текущий кадр**,
половина попадёт на чистые места, а на дефектных модель ловит лишь ~часть
(recall ≈ 3/4). На коротком окне это даёт **0 видимых детекций** — модель просто
не успевает попасть на дефект.

Поэтому VLM-плеер использует **manifest-guided семплинг**: на анализ подаются
кадры из **середины дефект-эпизодов** (по манифесту), а не случайный живой кадр.

> ✅ **Что остаётся реальным:** вердикт (`defect`/`clean`), тип, **бокс** и
> уверенность — это **настоящий вывод VLM**. Манифест влияет только на то, *какой
> кадр* показать модели, но **не подсказывает ей ответ** — модель сама может
> пропустить дефект (видно как `clean` в логе). Это честная демонстрация работы
> детектора, а не воспроизведение готовых меток.

Итог: детекция появляется каждые ~30 с с рамкой на сгустке (на Vulkan/Arc).
Для production guided-семплинг убирается — VLM анализирует реальный поток камеры
(см. [Возврат модели в систему](docs/fine-tuning.md#10-возврат-модели-в-систему)).

> ⚙️ **Ускорение на не-NVIDIA GPU (Vulkan).** По умолчанию LM Studio может выбрать
> CPU-движок (`llama.cpp ... avx2`) — тогда инференс ~50–77 с/кадр. Для Intel Arc /
> AMD переключите рантайм на **Vulkan**, чтобы выгрузить слои на iGPU:
> ```bash
> lms runtime select "llama.cpp-win-x86_64-vulkan-avx2"   # затем перезагрузить модель
> lms load google/gemma-4-12b-qat --gpu max
> ```
> На Intel Arc это дало **~28–31 с/кадр** (≈2–2.5× быстрее CPU).

> ⚠️ Даже с ускорением живой инференс — десятки секунд на кадр, поэтому детекция
> редкая. Это нормально: VLM-режим **доказывает**, что AI реально работает; для
> плавного показа используйте демо-режим. Быстрый real-time требует дискретного
> GPU (NVIDIA) — см. [Ограничения и перспективы](#-ограничения-и-перспективы).
