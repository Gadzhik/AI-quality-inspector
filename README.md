...
**VLM-режим** — живой инференс через LM Studio (нужен запущенный сервер с моделью):

```bash
python main_vlm.py
```

> ⚠️ VLM-режим на CPU/iGPU даёт ~50 с на кадр, поэтому детекция редкая (одна
> выборка в минуту). Это нормально — режим показывает, что AI **реально**
> работает; для плавного показа используйте демо-режим. Быстрый живой инференс
> требует дискретного GPU (NVIDIA).
